! Standalone CPU reproducer for the CanopyFluxesMod.F90 can_iter region.
!
...
!
! Build examples:
!   gfortran  -O3 -march=native -ffree-line-length-none can_iter_cpu_reproducer.f90 -o can_iter_cpu.exe
!   ifx       -O3 /QxHost can_iter_cpu_reproducer.f90 -o can_iter_cpu.exe
!   nvfortran -O3 -Minfo=all can_iter_cpu_reproducer.f90 -o can_iter_cpu.exe
!
! Run:
!   ./can_iter_cpu.exe [npts] [nrepeat] [nwarmup] [itmax]
!
! Example:
!   ./can_iter_cpu.exe 65536 20 3 40

module can_iter_cpu_reproducer_mod
...
contains
  ...
  subroutine can_iter_cpu(s, params, opts, itmax_canopy_fluxes)
    type(can_iter_state), intent(inout) :: s
    type(can_iter_params), intent(in) :: params
    type(can_iter_options), intent(in) :: opts
    ...
    do while (itlef <= itmax_canopy_fluxes .and. fn > 0)
      call FrictionVelocity(s, params, fn, s%filterp(1:fn), itlef + 1)
  ...
  end subroutine can_iter_cpu

  subroutine FrictionVelocity(s, params, fn, filtern, iter)
    type(can_iter_state), intent(inout) :: s
    type(can_iter_params), intent(in) :: params
  ...
  subroutine MoninObukIni(params, ur, thv, dthv, zldis, z0m, um, obu)
    type(can_iter_params), intent(in) :: params
...
end module can_iter_cpu_reproducer_mod

program can_iter_cpu_reproducer
  use, intrinsic :: iso_fortran_env, only : int64
  use can_iter_cpu_reproducer_mod
  implicit none
  ...
  do i = 1, nwarmup
    ...
    call can_iter_cpu(state, params, opts, itmax_canopy_fluxes)
  end do
  ...
  write(*,'(a)') 'can_iter CPU reproducer'
...
end program can_iter_cpu_reproducer
