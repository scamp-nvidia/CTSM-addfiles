! Standalone packed OpenACC GPU reproducer for the CanopyFluxesMod.F90 can_iter region.
!
...
!
! This packed variant keeps data resident for the whole run and uses one GPU
! kernel per canopy iteration. It avoids the per-iteration active-filter
! compaction and scalar copyback used by the first OpenACC version; convergence
! is tracked with an on-device active(:) mask instead.
!
! Build examples:
!   nvfortran -O3 -acc=gpu -Minfo=accel can_iter_openacc_packed_reproducer.f90 -o can_iter_openacc_packed.exe
!   nvfortran -O3 -acc=gpu -gpu=cc80 -Minfo=accel can_iter_openacc_packed_reproducer.f90 -o can_iter_openacc_packed.exe
!
! Run:
!   ./can_iter_openacc_packed.exe [npts] [nrepeat] [nwarmup] [itmax]
!
! Example:
!   ./can_iter_openacc_packed.exe 65536 20 3 40

module can_iter_openacc_packed_reproducer_mod
  ...
  type, public :: can_iter_state
    ...
    integer, allocatable :: filterp(:), filterp_next(:), fporig(:), active(:)
  ...
  public :: can_iter_openacc_packed
...
contains
  ...
  subroutine allocate_state(s, npts)
    ...
    allocate(s%filterp(npts), s%filterp_next(npts), s%fporig(npts), s%active(npts))
  ...
  subroutine enter_state_data(s)
    ...
    !$acc& s%filterp(:), s%active(:), s%column(:), s%gridcell(:), &
    !$acc& s%itype(:), s%snl(:), s%nmozsgn(:), &
  ...
  subroutine can_iter_openacc_packed(s, params, opts, itmax_canopy_fluxes)
    type(can_iter_state), intent(inout) :: s
    type(can_iter_params), intent(in) :: params
    type(can_iter_options), intent(in) :: opts
    integer, intent(in) :: itmax_canopy_fluxes

    integer :: itlef, iter_count
    integer :: f, p, c, g
    ...
    real(r8) :: vpd_kpa, rh_can, light, nitrogen, temp_stress, co2_fac
    real(r8) :: conductance, moisture

    do itlef = 0, itmax_canopy_fluxes
      iter_count = itlef + 1

      !$acc parallel loop gang vector present(s) private(p,c,g,zldis_local,zeta_local,cf,w, &
      !$acc& csoilb,ri,ricsoilc,csoilcn,wta,wtl,wtstem,wtshi,wtg0,rppdry,rpp,efpot,h2ocan, &
      !$acc& wtaq,wtlq,wtgq0,wtsqi,wtgaq,snow_depth_c,fsno_dl,elai_dl,rdl,dc1,dc2,efsh, &
      !$acc& erre,efeold,lw_grnd,dels,ecidif,tstar,qstar,thvstar,wc,vpd_kpa,rh_can,light, &
      !$acc& nitrogen,temp_stress,co2_fac,conductance,moisture)
      do f = 1, s%fn0
        p = s%filterp(f)

        if (s%active(p) /= 0) then
          c = s%column(p)
          g = s%gridcell(p)

          zldis_local = s%forc_hgt_u_patch(p) - s%displa(p)
          zeta_local = zldis_local / s%obu(p)
          if (zeta_local < -1.574_r8) then
            s%ustar(p) = vkc * s%um(p) / (log(1.574_r8 * (-s%obu(p)) / s%z0mv(p)) &
                 - StabilityFunc1(-1.574_r8) + StabilityFunc1(s%z0mv(p) / s%obu(p)) &
                 + 1.14_r8 * ((-zeta_local)**0.333_r8 - 1.574_r8**0.333_r8))
          else if (zeta_local < 0.0_r8) then
            s%ustar(p) = vkc * s%um(p) / (log(zldis_local / s%z0mv(p)) &
                 - StabilityFunc1(zeta_local) + StabilityFunc1(s%z0mv(p) / s%obu(p)))
          else if (zeta_local <= 1.0_r8) then
            s%ustar(p) = vkc * s%um(p) / (log(zldis_local / s%z0mv(p)) &
                 + 5.0_r8 * zeta_local - 5.0_r8 * s%z0mv(p) / s%obu(p))
          else
            s%ustar(p) = vkc * s%um(p) / (log(s%obu(p) / s%z0mv(p)) &
                 + 5.0_r8 - 5.0_r8 * s%z0mv(p) / s%obu(p) &
                 + (5.0_r8 * log(zeta_local) + zeta_local - 1.0_r8))
          end if

          zldis_local = s%forc_hgt_t_patch(p) - s%displa(p)
          zeta_local = zldis_local / s%obu(p)
          if (zeta_local < -0.465_r8) then
            s%temp1(p) = vkc / (log(0.465_r8 * (-s%obu(p)) / s%z0hv(p)) &
                 - StabilityFunc2(-0.465_r8) + StabilityFunc2(s%z0hv(p) / s%obu(p)) &
                 + 0.8_r8 * (0.465_r8**(-0.333_r8) - (-zeta_local)**(-0.333_r8)))
          else if (zeta_local < 0.0_r8) then
            s%temp1(p) = vkc / (log(zldis_local / s%z0hv(p)) &
                 - StabilityFunc2(zeta_local) + StabilityFunc2(s%z0hv(p) / s%obu(p)))
          else if (zeta_local <= 1.0_r8) then
            s%temp1(p) = vkc / (log(zldis_local / s%z0hv(p)) &
                 + 5.0_r8 * zeta_local - 5.0_r8 * s%z0hv(p) / s%obu(p))
          else
            s%temp1(p) = vkc / (log(s%obu(p) / s%z0hv(p)) &
                 + 5.0_r8 - 5.0_r8 * s%z0hv(p) / s%obu(p) &
                 + (5.0_r8 * log(zeta_local) + zeta_local - 1.0_r8))
          end if

          if (s%forc_hgt_q_patch(p) == s%forc_hgt_t_patch(p) .and. s%z0qv(p) == s%z0hv(p)) then
            s%temp2(p) = s%temp1(p)
          else
            zldis_local = s%forc_hgt_q_patch(p) - s%displa(p)
            zeta_local = zldis_local / s%obu(p)
            if (zeta_local < -0.465_r8) then
              s%temp2(p) = vkc / (log(0.465_r8 * (-s%obu(p)) / s%z0qv(p)) &
                   - StabilityFunc2(-0.465_r8) + StabilityFunc2(s%z0qv(p) / s%obu(p)) &
                   + 0.8_r8 * (0.465_r8**(-0.333_r8) - (-zeta_local)**(-0.333_r8)))
            else if (zeta_local < 0.0_r8) then
              s%temp2(p) = vkc / (log(zldis_local / s%z0qv(p)) &
                   - StabilityFunc2(zeta_local) + StabilityFunc2(s%z0qv(p) / s%obu(p)))
            else if (zeta_local <= 1.0_r8) then
              s%temp2(p) = vkc / (log(zldis_local / s%z0qv(p)) &
                   + 5.0_r8 * zeta_local - 5.0_r8 * s%z0qv(p) / s%obu(p))
            else
              s%temp2(p) = vkc / (log(s%obu(p) / s%z0qv(p)) &
                   + 5.0_r8 - 5.0_r8 * s%z0qv(p) / s%obu(p) &
                   + (5.0_r8 * log(zeta_local) + zeta_local - 1.0_r8))
            end if
          end if
          ...
          if (opts%use_cn .and. opts%use_c13) then
            s%canopy_cond(p) = s%canopy_cond(p) + 1.0e-12_r8 * real(mod(p, 7), r8)
          end if
          ...
          if (iter_count > itmin) then
            s%dele(p) = abs(s%efe(p) - s%efeb(p))
            s%efeb(p) = s%efe(p)
            s%det(p) = max(s%del(p), s%del2(p))
            s%num_iter(p) = real(iter_count, r8)
            if (s%det(p) < dtmin .and. s%dele(p) < dlemin) s%active(p) = 0
          end if
        end if
      end do
    end do
  end subroutine can_iter_openacc_packed

  subroutine can_iter_openacc_filter_reference(s, params, opts, itmax_canopy_fluxes)
    type(can_iter_state), intent(inout) :: s
    type(can_iter_params), intent(in) :: params
    type(can_iter_options), intent(in) :: opts
    integer, intent(in) :: itmax_canopy_fluxes
    ...
    !$acc end kernels
  end subroutine can_iter_openacc_filter_reference
...
end module can_iter_openacc_packed_reproducer_mod

program can_iter_openacc_packed_reproducer
  use, intrinsic :: iso_fortran_env, only : int64
  use can_iter_openacc_packed_reproducer_mod
  implicit none
  ...
  do i = 1, nwarmup
    ...
    call can_iter_openacc_packed(state, params, opts, itmax_canopy_fluxes)
    !$acc wait
  end do
  ...
  write(*,'(a)') 'can_iter packed OpenACC GPU reproducer'
...
end program can_iter_openacc_packed_reproducer
